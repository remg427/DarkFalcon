...
Start-Transcript -Path 'D:\Scripts\AttackIQ\Logs\FDResults.txt' -Append
# Script for Automation of Get-FireDrillResults from the AttackIQ FireDrill Results API

$FDResults = Get-FireDrill -attackIQAPI Results
$SplunkReturnCount = '1000'

If (($FDResults) -notlike $null) {
    $SplunkResult = Set-FireDrillSplunkSearch -APISplunkSearch Results -APIResultSource DEFAULT -APIResults $FDResults -SplunkReturnCount $SplunkReturnCount
} else {
...
Write-host "SAMPLE SET: 5 FireDrill Results" -ForegroundColor Cyan
